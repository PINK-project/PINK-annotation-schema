...
from tripper.datadoc import store, get_keywords, search, acquire
...
# Make list columns as lists. Use regex to separate strings on ',', ' ', ';' and '|' and remove extra whitespace.
def split_to_list(value):
    if pd.isna(value) or str(value).strip() == "":
        return []  # Return empty list if empty or NaN
...
list_columns = termdefs[termdefs['SingleValue'] == False]['Property'].tolist()
...
sw = sw.drop(columns=["indicator", 
                      #"releaseDate", # Not handled properly
                      ])
...
#swdocumentation2 = TableDoc.parse_csv(
#        'sw_clean_2.csv',
#        keywords=kw,
#        #baseiri='https://w3id.org/pink/',
#        prefixes={'mw': 'https://modelwave.it/ontology/','rights': 'http://publications.europa.eu/resource/authority/access-right/'}
#        )
#swdocumentation2.save(ts)


# Test dataset form Joh
#jsonld_doc = json.load(open('validation/tests/dataset.jsonld'))
#d = store(ts, jsonld_doc, keywords=kw)

#datasets = search(ts)
#acquire(ts, search(ts)[0])
