...
| pink:Dataset         | A collection of data, published or curated by a single agent, and available for access or download in one or more representations. | A collection of (more than one) datum, published or curated by a single agent, and available for access or download in one or more representations. |
| pink:Datum           | A self-consistent encoded data entity. | Datum is the building block of datasets. |
...
The following classes are included in the PINK Annotation Schema to provide a structure, but are not intended to be used directly.
